...
func _ready() -> void:
	# Fallback: find player by group if the direct path is missing.
	if player == null:
		var p := get_tree().get_first_node_in_group("player")
		if p:
			player = p

	# Ensure loop flags are correct for each animation.
...
func _on_hurt_box_body_entered(body: Node2D) -> void:
	if body.is_in_group("bullets"):
		_last_hit_from = body.global_position  # Remember last impact source.
		take_damage(10)
		Global.add_score(30)
...
func _die() -> void:
	...
	# Spawn blood once, angled away from impact source.
