...
const DASH_DURATION: float = 0.3  # Duration for dash boost

# Enemy stats/state
var hp: int = 200
var threshold: float = 2.0          # Distance to target before stopping
var follow: bool = false

# State flags
var death: bool = false
var dashing: bool = false
var dash_ready: bool = true         # Controls dash re-trigger when player exits/re-enters detection area
...
func _ready() -> void:
	# Start tracking the player's position; group used by your damage logic.
	navigation_agent_2d.target_position = player.global_position
	add_to_group("bullets")


func make_path() -> void:
	# Refresh path toward the player's current position.
	navigation_agent_2d.target_position = player.global_position
...
func _physics_process(delta: float) -> void:
	...
	# Follow only when the player is detected.
	if follow:
...
func update_animation(move_dir: Vector2) -> void:
	...
	# Flip horizontally based on dominant horizontal movement.
...
func _on_timer_timeout() -> void:
	# Periodic path refresh (via Timer).
	make_path()


func _on_dash_detect_area_body_entered(body: Node2D) -> void:
	# Start dash when player enters detection zone (if ready).
...
func _on_dash_detect_area_body_exited(body: Node2D) -> void:
	# Allow dashing again once player leaves the zone.
	if body.name == "Player":
...
func _on_hurt_box_body_entered(body: Node2D) -> void:
	# Projectile hit → take damage and award score.
...
func die() -> void:
	# Spawn blood, play death anim, then free.
	if death:
		# Choose impact source: last bullet pos, else player pos (if valid), else self.
